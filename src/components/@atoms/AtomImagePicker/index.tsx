...
import React, { Component, PureComponent, RefObject, useEffect, useState } from 'react';
import {
  ActivityIndicator,
  BackHandler,
  Dimensions,
      ...
      <View style={styles.defaultVideoBg}>
        <View style={styles.defaultVideoContainer}>
          <Text style={{ fontSize: 6, color: 'white' }}>
            {new Date(this.props.item.asset.duration * 1000).toISOString().slice(11, 19)}
          </Text>
        </View>
      </View>
    );
...
export class ImagePickerCarousel extends Component<ImagePickerCarouselProps> {
  ...
  selectedImage(checked: boolean, selected: SelectedAsset) {
    if (this.props.max && this.state.selectedAssets.size >= this.props.max && checked) {
      return false;
    }
  ...
  async fillStartImages() {
    ...
    if (fetched && this.state.data.length < needFetch) await this.fillStartImages();
  }
...
function ScrollTime({
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const days = [...new Set(data.map((s) => getDay(s.asset.modificationTime)))];
  ...
  useEffect(() => {
    if (!isMoving && button) {
      const checked = data.findIndex(
        (i) => getDay(i.asset.modificationTime) == getDay(button.date.getTime()) && i.isChecked()
      );
      const normal =
        checked == -1 &&
...
function DefaultAlbum(props: AlbumData) {
  return (
    ...
    >
      <Image style={styles.defaultAlbumImage} source={{ uri: props.thumb.uri }} />
      ...
      {props.view == 'gallery' && (
        <>
          ...
          {props.imagesPicked > 0 && (
            <>
              {props.multiple && (
                <Text style={{ fontSize: 20 }}>Selected {props.imagesPicked} images</Text>
              )}
...
export function AtomImagePicker(props: ImagePickerProps) {
  const [status, requestPermission] = MediaLibrary.usePermissions();

  const [albums, setAlbums] = useState<AlbumData[]>();
  const [selectedAlbum, setSelectedAlbum] = useState<Album | undefined>(props.selectedAlbum);
  const [selectedAssets, setSelectedAssets] = useState<Asset[]>(props.selected ?? []);
